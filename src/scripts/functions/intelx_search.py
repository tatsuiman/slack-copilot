...
class intelx:
    ...
    def treeview(self, id, bucket):
        ...
        h = {"x-key": self.API_KEY, "User-Agent": self.USER_AGENT}
        r = requests.get(
            self.API_ROOT + f"/file/view?f=13&storageid={id}&bucket={bucket}", headers=h
        )
...
def run(keyword: str) -> str:
    ...
    if result:
        for record in result["records"]:
            resp += (
                f"systemid: {record['systemid']},"
                f"size: {record['size']},"
                f"type: {record['type']},"
                f"media: {record['media']},"
                f"added: {record['added']},"
                f"date: {record['date']},"
                f"name: {record['name']},"
                f"description: {record['description']},"
                f"bucket: {record['bucket']},"
                f"accesslevel: {record['accesslevel']}\n"
            )
        return resp
    else:
