...
class AssistantAPIClient:
    ...
    def create_run(self, thread_id, assistant_id, additional_instructions=None):
        ...
        for i in range(timeout):
            ...
            if run.status == "requires_action":
                ...
                for tool_call in run.required_action.submit_tool_outputs.tool_calls:
                    ...
                    log_callback(log_messages, progress)
                    result = self.process_function_call(
            ...
            if run.status == "in_progress":
                resp = self.client.beta.threads.runs.steps.list(
                    thread_id=thread_id,
                    run_id=run.id,
                )
                ...
                if len(list(resp)) > 0:
                    ...
                    for step in run_steps:
                        ignore_ids = self.process_step(step, callback, ignore_ids)

                if current_message_len < len(list(messages)):
                    current_message_len = len(list(messages))
                    self.process_message(messages, callback)
    ...
    def process_function_call(self, function_name, function_arguments):
        # プラグインとその優先度を格納するリスト
    ...
    def process_step(self, step, callback, ignore_ids=[]):
    ...
    def process_message(self, messages, callback):
        res = {}
            ...
            if message.role == "assistant":
                ...
                for message_content in message.content:
                    ...
                    if message_content.type == "text":
                        ...
                        for annotation in message_content.text.annotations:
                            ...
                            if annotation.type == "file_path":
                                ...
                                annotation_file = self.retrieve_file(
                                    file_id, directory="/tmp"
                                )
                    ...
                    if message_content.type == "image_file":
                        ...
                        png_file = self.retrieve_file(file_id, directory="/tmp")
    ...
    def retrieve_file(self, file_id, directory="/tmp"):
    ...
    def generate_json(self, text, system_prompt, key_name):
        chat = ChatOpenAI(model=BASE_MODEL, api_key=self.api_key).bind(
    ...
    def generate_title(self, text):
        ...
        return self.generate_json(text, system_prompt, "title")
